...
describe("P23 interactive PTY", () => {
  windowsTest("runs an interactive Windows ConPTY session with input and resize", async () => {
    ...
    try {
      ...
      expect(manager.list().some((entry) => entry.id === started.id)).toBe(true);
    } finally {
...
async function waitForText(
  ...
  while (Date.now() < deadline) {
    ...
    if (output.state !== "running")
      throw new Error(`PTY exited before '${expected}' appeared: ${output.text}`);
    await delay(25);
  }
  const output = manager.read(ptyId, 0, 100_000);
