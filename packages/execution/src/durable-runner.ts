...
export class DurableRunner {
  ...
  public recover(): void {
    for (const attempt of this.store.activeAttempts()) this.reconcile(attempt);
    this.pump();
  }

  public submit(input: SubmitCommand): {task: ExecutionTask; reused: boolean} {
    if (this.closed) throw new Error("DAEMON_CLOSED");
    if (!input.idempotencyKey?.trim() || !input.workspace?.trim()) throw new Error("INVALID_INPUT: key and workspace required");
    if (!input.command || !["direct", "shell"].includes(input.command.kind)) throw new Error("INVALID_INPUT: command required");
    if (input.command.kind === "direct" && (!input.command.file || !Array.isArray(input.command.args) || input.command.args.some(arg => typeof arg !== "string")))
      throw new Error("INVALID_INPUT: direct file and args required");
    if (input.command.kind === "shell" && (!input.command.command || !["cmd", "powershell"].includes(input.command.shell)))
      throw new Error("INVALID_INPUT: shell command invalid");
    const timeoutMs = input.timeoutMs ?? 120_000;
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 3_600_000) throw new Error("INVALID_INPUT: timeoutMs invalid");
    ...
    const inputDigest = createHash("sha256").update(JSON.stringify(snapshot)).digest("hex");
    const result = this.store.accept({
      owner: os.userInfo().username, workspace: input.workspace, operation: "command.run",
      idempotencyKey: input.idempotencyKey, inputDigest, definitionSnapshot: snapshot,
    });
  ...
  public output(taskId: string, stream: OutputStream, cursor = 0, maxBytes = 32 * 1024) {
    ...
    return new OutputSpool(this.spoolRoot, task.attemptId).page(stream, cursor, maxBytes);
  }

  private verifyManifest(attempt: DispatchAttempt): {path: string; manifest: CompletionManifest} | null {
    ...
    if (manifest.attemptId !== attempt.attemptId ||
        ...
        !Number.isSafeInteger(manifest.stdoutBytes) || !Number.isSafeInteger(manifest.stderrBytes) ||
        manifest.stdoutBytes < 0 || manifest.stderrBytes < 0) return null;
    for (const stream of ["stdout", "stderr"] as const) {
      const file = path.join(directory, `${stream}.spool`);
      const bytes = stream === "stdout" ? manifest.stdoutBytes : manifest.stderrBytes;
      const expected = stream === "stdout" ? manifest.stdoutSha256 : manifest.stderrSha256;
  ...
  private launch(attempt: DispatchAttempt): void {
    ...
    let ready = false;
    let received = "";
    const guard = setTimeout(() => { if (!ready) worker.kill(); }, 10_000);
    worker.stdout?.on("data", (chunk: Buffer) => {
      if (ready) return;
      received += chunk.toString("utf8");
      if (received.length > 256) { worker.kill(); return; }
    ...
    worker.once("error", () => { /* close/reconciliation handles unknown outcomes */ });
    worker.once("close", () => {
      clearTimeout(guard);
      ...
      if (!this.reconcile(attempt)) {
        ...
        this.store.interruptUnverified(attempt.taskId, "worker exited without a valid completion manifest; child effects may be unknown");
      }
      this.pump();
    });
  }
