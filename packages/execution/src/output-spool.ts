...
// Windows scanners can briefly deny a just-written spool file during final fsync/hash.
// Only the detached worker blocks here; never report completion without a flushed manifest.
function withSpoolFileRetry<T>(operation: () => T): T {
...
export class OutputSpool {
  ...
  private readonly bytes: Record<OutputStream, number> = {
    stdout: 0,
    stderr: 0,
  };
  ...
  public constructor(
    ...
    if (!/^attempt_[a-f0-9-]{36}$/.test(attemptId))
      throw new Error(
        "INVALID_INPUT: attemptId must be a generated UUID attempt identifier",
      );
    ...
    if (existsSync(manifest)) {
      this.droppedBytes = (
        JSON.parse(readFileSync(manifest, "utf8")) as CompletionManifest
      ).droppedBytes;
      this.finalized = true;
    }
  }

  private file(stream: OutputStream): string {
    return path.join(this.root, `${stream}.spool`);
  }

  public append(stream: OutputStream, chunk: Buffer): void {
    if (this.finalized)
      throw new Error("OUTPUT_FINALIZED: cannot append after completion");
    const available = Math.max(
      0,
      this.maxBytes - this.bytes.stdout - this.bytes.stderr,
    );
  ...
  public page(
    ...
    if (
      ...
      cursor < 0 ||
      cursor > this.bytes[stream]
    )
      throw new Error(
        "OUTPUT_CURSOR_INVALID: cursor is outside retained spool",
      );
    ...
    if (length > 0) {
      ...
      try {
        ...
        while (done < length) {
          ...
          if (count === 0)
            throw new Error("OUTPUT_TRUNCATED: spool changed during read");
          done += count;
        }
      } finally {
        closeSync(fd);
    ...
    return {
      ...
      cursor,
      nextCursor,
      complete: this.finalized && nextCursor === this.bytes[stream],
  ...
  public finalize(
    ...
    signal: string | null,
    canceled = false,
  ): { path: string; manifest: CompletionManifest } {
    if (this.finalized)
      throw new Error("OUTPUT_FINALIZED: completion manifest already exists");
    for (const stream of ["stdout", "stderr"] as const) {
      const file = this.file(stream);
    ...
    const hashFile = (stream: OutputStream): string => {
      const file = this.file(stream);
      if (!existsSync(file)) return createHash("sha256").digest("hex");
      return withSpoolFileRetry(() => {
    ...
    const manifest: CompletionManifest = {
      ...
      exitCode,
      signal,
      stdoutBytes: this.bytes.stdout,
      stderrBytes: this.bytes.stderr,
      droppedBytes: this.droppedBytes,
      outputState: this.droppedBytes > 0 ? "partial" : "complete",
      stdoutSha256: hashFile("stdout"),
      stderrSha256: hashFile("stderr"),
    ...
    if (existsSync(destination))
      throw new Error("OUTPUT_FINALIZED: manifest already exists");
    withSpoolFileRetry(() => renameSync(temporary, destination));
  ...
  public manifest(): CompletionManifest | null {
    ...
    return existsSync(file)
      ? (JSON.parse(readFileSync(file, "utf8")) as CompletionManifest)
      : null;
  }
}
