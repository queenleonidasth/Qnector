...
async function rpc(port: number, method: string, params: Record<string, unknown> = {}) {
  ...
  return (data ? JSON.parse(data.slice(5).trim()) : JSON.parse(text)) as {
    result?: {tools?: Array<{name: string}>; structuredContent?: {
      ok: boolean; data?: Record<string, unknown>; error?: {code: string};
    }};
  };
}
async function call(port: number, action: string, input: Record<string, unknown> = {}) {
  const response = await rpc(port, "tools/call", {name: "tasks", arguments: {action, ...input}});
...
describe("opt-in durable MCP task facade", () => {
  it("leaves the legacy eight-tool contract unchanged without opt-in", async () => {
    ...
    const runtime = new QnectorRuntime({config: {...defaultConfig(workspace), localPort: port}});
    ...
    expect(listed.result?.tools?.map(tool => tool.name)).not.toContain("tasks");
  });
  ...
  it("accepts once, auto-yields a handle, reconnects for result and exposes explicit cancel", async () => {
    ...
    runtimes.push(runtime);
    await runtime.start({port});
    expect((await rpc(port, "tools/list")).result?.tools?.map(tool => tool.name)).toContain("tasks");
    const doctor = await call(port, "doctor");
    expect(doctor?.ok).toBe(true);
    expect(doctor?.data).toMatchObject({state: "ready", protocol: 1,
    ...
    for (const action of ["get", "wait", "output", "result", "inspect", "cancel"]) {
      const denied = await executeDurableTask(daemonRoot, foreignWorkspace,
        {action, taskId, stream: "stdout", waitTimeoutMs: 0});
      expect(denied).toMatchObject({ok: false, error: {code: "TASK_NOT_FOUND"}});
    }
    ...
    expect(waited?.data).toMatchObject({taskId, state: "succeeded", nextAction: "result"});
    expect((await call(port, "result", {taskId}))?.data).toMatchObject({state: "succeeded"});
    expect((await call(port, "output", {taskId, stream: "stdout"}))?.data).toMatchObject({text: "DONE\n", complete: true});
    expect(readFileSync(marker, "utf8")).toBe("ONE\n");
    const canceled = await call(port, "start", {
      idempotencyKey: "cancel-this", waitTimeoutMs: 0,
    ...
    if (!cancelResponse?.ok) throw new Error(`CANCEL_RESPONSE: ${JSON.stringify(cancelResponse)}`);
    expect(cancelResponse?.data?.state)
      .toMatch(/canceled|canceling/);
    expect((await call(port, "wait", {taskId: cancelId, waitTimeoutMs: 8_000}))?.data)
