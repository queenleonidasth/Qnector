...
type RpcResponse = {id: number; result?: {tools?: Array<{name: string; inputSchema: unknown}>; structuredContent?: {ok: boolean; data?: Record<string, unknown>}}; error?: unknown};
class StdioRpc {
  ...
  public constructor() {
    this.lines.on("line", line => {
      ...
      } catch (error) { this.errors.push(error as Error); }
    });
  }
  public request(method: string, params: Record<string, unknown> = {}): Promise<RpcResponse> {
    const id = this.nextId++;
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {this.pending.delete(id); reject(new Error(`STDIO_TIMEOUT_${method}`));}, 9_000);
      this.pending.set(id, response => {clearTimeout(timeout); resolve(response);});
  ...
  public async initialize(): Promise<void> {
    const result = await this.request("initialize", {protocolVersion: "2025-06-18", capabilities: {}, clientInfo: {name: "stdio-parity", version: "1"}});
    if (result.error) throw new Error(`STDIO_INIT_FAILED: ${JSON.stringify(result.error)}`);
    this.notify("notifications/initialized");
  }
  public close(): void { this.lines.close(); this.input.destroy(); this.output.destroy(); }
}
...
async function http(port: number, method: string, params: Record<string, unknown> = {}): Promise<RpcResponse> {
...
const task = (action: string, additional: Record<string, unknown> = {}) =>
  ({name: "tasks", arguments: {action, ...additional}});
...
afterEach(async () => {
  ...
  runtime = undefined; stdio = undefined; daemon = undefined; root = undefined;
});
...
describe("P4 HTTP and stdio share one durable execution backend", () => {
  it("advertises identical tools and retrieves HTTP-submitted work after stdio disconnect without replay", async () => {
    ...
    runtime = new QnectorRuntime({
      config: {...defaultConfig(root), localPort: port}, configFile: path.join(root, "config.json"),
      logger: new ActivityLogger(path.join(root, "activity.jsonl")), durableDaemonRoot: daemonRoot,
    });
    await runtime.start({port});
    stdio = new StdioRpc();
    await runtime.startStdio({transport: stdio.transport, onerror: error => stdio?.errors.push(error)});
    ...
    expect(stdioTools?.map(tool => tool.name)).toEqual(httpTools?.map(tool => tool.name));
    expect(stdioTools?.find(tool => tool.name === "tasks")?.inputSchema)
      .toEqual(httpTools?.find(tool => tool.name === "tasks")?.inputSchema);
    const marker = path.join(root, "effect.txt");
    const request = task("start", {idempotencyKey: "shared-across-transports", waitTimeoutMs: 0,
      command: {kind: "direct", file: process.execPath, args: ["-e",
        "setTimeout(()=>{require('fs').appendFileSync(process.argv[1],'ONE\\n');console.log('PARITY_OK')},750)", marker]}});
    const accepted = (await http(port, "tools/call", request)).result?.structuredContent;
    ...
    stdio.close();
    const waited = (await http(port, "tools/call", task("wait", {taskId, waitTimeoutMs: 6_000}))).result?.structuredContent;
    expect(waited?.data).toMatchObject({taskId, state: "succeeded"});
    const result = (await http(port, "tools/call", task("result", {taskId}))).result?.structuredContent;
    expect(result?.data).toMatchObject({state: "succeeded"});
    const output = (await http(port, "tools/call", task("output", {taskId, stream: "stdout"}))).result?.structuredContent;
