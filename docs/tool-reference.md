...
| Tool        | Actions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `system`    | `parallel`, `info`, `status`, `build_info`, `release_status`, `context_snapshot`, `processes`, `process_info`, `find_process`, `ports`, `doctor`, `mcp_servers`, `mcp_tools`, `mcp_call`, `skills_status`, `skills_list`, `skills_match`, `skill_get`, `everything_status`, `which`, `search_files`, `env`, `open_path`, `open_url`, `clipboard_read`, `clipboard_write`, `toast`, `screen_capture`, `window_list`, `window_focus`                                                                                                |
...
Qnector can also act as an MCP client for local upstream servers. Configure stdio servers in `%APPDATA%\\Qnector\\external-mcp.json`; `system.mcp_servers` lists configured upstreams without exposing environment values, `system.mcp_tools` discovers/filter tools and their JSON schemas, and `system.mcp_call` invokes an upstream tool. Optional `dpapiEnv` entries map environment-variable names to Windows DPAPI-encrypted files; missing secret files are omitted so an upstream can still expose unauthenticated tools. Qnector decrypts those values only when spawning the upstream process, and `mcp_call` activity logging records argument keys rather than argument values.
