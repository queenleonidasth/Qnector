...
| Tool        | Actions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `system`    | `info`, `status`, `build_info`, `release_status`, `context_snapshot`, `processes`, `process_info`, `find_process`, `ports`, `doctor`, `mcp_servers`, `mcp_tools`, `mcp_call`, `skills_status`, `skills_list`, `skills_match`, `skill_get`, `everything_status`, `which`, `search_files`, `env`, `open_path`, `open_url`, `clipboard_read`, `clipboard_write`, `toast`, `screen_capture`, `window_list`, `window_focus`                                                                                                            |
| `workspace` | `get`, `set`, `list_recent`, `tree`, `list`, `glob`, `grep`, `stat`, `summary`, `diagnostics`, `document_symbols`, `document_outline`, `workspace_symbols`, `definition`, `references`, `hover`, `rename_locations`, `semantic_search`, `lsp_status`, `lsp_document_symbols`, `lsp_workspace_symbols`, `lsp_definition`, `lsp_references`, `lsp_hover`, `watch`, `watch_events`, `unwatch`, `wait_for_file`, `wait_for_change`                                                                                                                        |
| `files`     | `read`, `read_many`, `preview`, `inspect`, `extract_text`, `render`, `document_query`, `document_replace_text`, `write`, `append`, `replace`, `multi_edit`, `apply_patch`, `mkdir`, `move`, `copy`, `delete`, `hash`                                                                                                                                                                                                                                                                                                              |
| `process`   | `run`, `start`, `output`, `stdin`, `stop`, `list`, `kill_tree`, `wait_for_exit`, `wait_for_output`, `wait_for_port`, `task_start`, `task_get`, `task_list`, `task_cancel`, `workflow_save`, `workflow_list`, `workflow_get`, `workflow_start`, `workflow_run`, `workflow_wait`, `workflow_result`, `workflow_document_batch`, `workflow_status`, `workflow_runs`, `workflow_cancel`, `workflow_resume`                                                                                                                            |
...
Qnector can also act as an MCP client for explicitly configured upstream servers. Configure either `command`/`args` (stdio) or `url` (Streamable HTTP) per server in `%APPDATA%\\Qnector\\external-mcp.json`; `system.mcp_servers` lists upstreams without exposing secret values, `system.mcp_tools` filters their tool schemas on demand, and `system.mcp_call` invokes a named tool. HTTP requires HTTPS except for loopback; URL credentials, query strings, fragments and redirects are rejected. For owner-provided bearer credentials, `bearerTokenFile` can point to a Windows DPAPI-encrypted file; no ChatGPT/Khai-Hub OAuth credentials are extracted or shared automatically. stdio retains optional `dpapiEnv` entries for child environment variables. `mcp_call` activity logging records argument keys rather than values. The upstream's existing authorization and Qnector's tool-call permissions both apply. See `docs/khai-hub-mcp-integration.md` for activation and limitations.
...
The `system.parallel` action was removed in v0.4.35. Issue separate top-level MCP tool calls so ChatGPT can observe each result and lifecycle. For long commands, use `process.start` followed by bounded `process.output` polling. Workflow graphs still exist but are not individual ChatGPT tool events.

Persistent PowerShell commands share a pool of at most four workers per executable. Independent `process.run` calls can therefore execute concurrently even when they need PowerShell syntax. Additional calls queue on the least busy worker. Each call sets its own working directory and returns its own output; do not depend on shell variables or imported modules surviving between calls. Use one command or a PTY session for dependent shell state. The pool is execution capacity, not an LLM subagent runtime or a scheduler for dependent workflows.
...
`workspace.diagnostics` follows `tsconfig.json` projects/references and returns structured diagnostics with 1-based locations. `document_symbols`, `document_outline`, `workspace_symbols`, `definition`, `references`, `hover`, and `rename_locations` use the TypeScript Language Service. `document_outline` returns only top-level declarations as compact name/kind/line/endLine records; use `document_symbols` when nested symbols and detailed previews are needed. Both support bounded `maxResults` and `offset`. `workspace_symbols` accepts a query and searches symbols across the selected project graph. Rename discovery is read-only; actual edits still go through `files`.
...
The index reuses unchanged files, invalidates on modification/change time or size, and shares concurrent builds for the same search root. Matching splits snake_case, camelCase and acronym identifiers, preserves Unicode marks, and segments Thai words. Ranking requires a shared normalized word and combines query-word coverage with vector similarity; it is lexical relevance, not a calibrated probability or cross-language understanding. `import` and `export` remain searchable. Short nonempty files are included.

Use `maxResults` (1-100), `offset`, and `minScore` (0-1) to page or filter matches. The response includes `totalMatches` within the indexed subset, optional `nextOffset` (also returned as `meta.nextCursor`), and cache read/reuse counts. `truncated` indicates either another result page or an incomplete index; `indexTruncated` specifically indicates incomplete coverage. Pagination assumes files remain unchanged; compare `fingerprint` between pages if consistency matters.

Search bounds are 2 MiB per file, 10,000 chunks per index, 8,000 characters per chunk, 50,000 scanned directory entries, and three cached roots. Oversized, unreadable or binary text candidates are skipped and reported in `skippedFiles`; `indexedFiles` excludes those skipped files. Clipped chunks, unreadable directories, and exhausted scan/file/chunk budgets set `indexTruncated`. Narrow `path` to search a specific subtree when a project exceeds these limits. For exhaustive literal matching, use `workspace.grep`.

Fallback `system.search_files` accepts quoted paths such as `path:"C:\\Monthly Reports" report-*.xlsx`. Filename wildcards match through the end of the filename, so `*.xlsx` excludes `.xlsx.bak` files. Unmatched quotes return `INVALID_INPUT`.
...
`process.workflow_save/list/get/start/run/status/runs/wait/result/cancel/resume` adds a persistent multi-step workflow layer without creating a ninth MCP tool. Definitions live under `.qnector/workflows`; run state lives under `.qnector/workflow-runs`; complete per-step results live under `.qnector/workflow-results/<runId>/`. Saved workflows remain sequential by default for compatibility, while `mode: "graph"` enables dependency-aware scheduling with `dependsOn` and bounded `maxConcurrency` (1-8). Steps can run commands, existing Qnector tools, wait-for-port, wait-for-file, wait-for-change or bounded delays.

Graph steps may declare `resourcePaths` to prevent concurrent mutations of overlapping files/directories (or named `key:` resources) while unrelated work continues. They may also declare `outputs`; Qnector verifies those paths exist before the step is marked successful. Tool steps can use `expect.contains` for deterministic result assertions. A failed branch skips only downstream dependents; independent branches are allowed to finish and retain their results.

`workflow_run` starts a one-shot workflow without first saving a definition. `workflow_wait` waits on the actual execution lifecycle without polling, and timeout does not cancel the run. `workflow_result` returns either the compact result index or the complete stored result for one step, including command stdout/stderr or a bounded tool result. `workflow_cancel` aborts active command executors and does not report `canceled` until those executors have settled. If a runtime dies while a command/tool mutation has unknown outcome, recovery marks the step/run `interrupted`; `workflow_resume` refuses to replay that mutation unless the caller explicitly sets `replayInterrupted: true` or the step was declared `replay: "safe"`.

`workflow_document_batch` is the first artifact recipe. Each document performs prepare/inspect, one or more ordered OOXML text edits, validation by re-extracting text, optional render, then a final collect barrier; separate documents can execute concurrently while duplicate output destinations are rejected. Rendering is opt-in because the current `files.render` implementation renders PDF pages; normal DOCX/PPTX edit batches should leave `render` false unless the destination is a renderable format.

## Document intelligence

`files.inspect`, `extract_text`, `render` and `document_query` provide structured local document handling. Native support includes PDF text/page rendering, DOCX text/properties, XLS/XLSX sheets, CSV/text, JSON structure, ZIP listings and SQLite schema/read queries. When Microsoft MarkItDown is installed, `extract_text` also routes PPTX, OpenDocument files, EPUB, RTF, Outlook/email files, Jupyter notebooks, common images and common audio formats through that optional local provider. `files.document_replace_text` performs exact text-run replacement inside DOCX and PPTX OOXML packages without flattening the document; text split across multiple OOXML runs intentionally requires a structure-aware editor instead. PDF render results use the normal MCP image-attachment path so ChatGPT can visually inspect a selected page.
...
Start with `computer.windows`, then `find` or bounded `inspect` to obtain session-scoped element IDs. Supported semantic patterns include Invoke, Value, SelectionItem, Toggle, ExpandCollapse, ScrollItem and RangeValue. `wait` supports bounded state waits such as exists/not_exists/enabled/disabled/value_equals. Non-finite native bounds are normalized before JSON serialization. Stale elements return `ELEMENT_STALE` and should be re-found. If the persistent UIA helper loses its response, Qnector may retry read-only observation actions once after restarting the helper, but it never blindly replays mutations such as invoke/toggle/set-value; those return `UIA_OUTCOME_UNKNOWN` so the caller can inspect state before deciding whether to retry.
...
- Semantic/browser locators preserve Playwright strictness when `index` is omitted. If multiple elements match, the action fails as ambiguous instead of silently choosing the first element; supply an explicit `index` only after inspecting the matches.
...
## Desktop startup and splash

The desktop keeps the heavy MCP runtime off the first-window critical path. After config is loaded, a lightweight self-contained splash is shown for normal visible startup while the main BrowserWindow loads hidden. The main window is revealed only on Electron `ready-to-show`; runtime import/start begins at that handoff so a usable renderer receives priority over background initialization. Startup performance marks include `splash-created`, `window-created`, `renderer-loaded`, `renderer-ready-to-show`, `runtime-start-queued`, `mcp-runtime-imported`, and `mcp-runtime-ready`. `accept:startup` launches a hidden Electron probe and fails if first renderer readiness exceeds its bounded regression budget.
