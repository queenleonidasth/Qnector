...
async function until<T>(
  ...
  for (;;) {
    ...
    if (Date.now() > deadline)
      throw new Error(`TIMED_OUT: ${JSON.stringify(result)}`);
...
describe("P2 named-pipe daemon", () => {
  it("arbitrates a single owner and rejects requests without its token", async () => {
    ...
    await daemon.start();
    expect(await daemonRequest(root, { action: "ping" })).toMatchObject({
      ok: true,
      data: { state: "ready" },
    });
    ...
    const denied = await new Promise<string>((resolve, reject) => {
      ...
      socket.on("connect", () =>
        socket.write(
          JSON.stringify({ action: "ping", token: "forged" }) + "\n",
        ),
      );
  ...
  (process.platform === "win32" ? it : it.skip)(
    "waits for a transient named-pipe owner without opening a second journal writer",
    async () => {
      const { root, daemon } = fixture();
      const blocker = net.createServer();
      await new Promise<void>((resolve, reject) => {
        blocker.once("error", reject);
        blocker.listen(daemonSocketPath(root), () => {
          blocker.off("error", reject);
          resolve();
        });
      });
      try {
        const startup = daemon.start().then(
          () => true,
          (error) => error,
        );
        await new Promise((resolve) => setTimeout(resolve, 150));
        expect(existsSync(path.join(root, "execution.sqlite"))).toBe(false);
  ...
  it("reconnects after daemon closes, keeps the worker and retrieves exactly one result", async () => {
    ...
    expect((await daemonRequest(root, input)).data).toMatchObject({
      taskId,
      reused: true,
    });
    await until(
    ...
    const output = await daemonRequest(root, {
      action: "output",
      taskId,
      stream: "stdout",
    });
