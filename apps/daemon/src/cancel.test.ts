...
async function until<T>(read: () => Promise<T>, done: (value: T) => boolean, ms = 12_000): Promise<T> {
...
describe("P2 explicit cancellation", () => {
  it("cancels a running command and grandchild without reporting canceled before the kill manifest", async () => {
    ...
    const grandchild = "setTimeout(()=>require('fs').appendFileSync(process.argv[1],'late'),1800)";
    ...
    const accepted = await daemonRequest(root, {
      action: "submit", workspace: root, idempotencyKey: "cancel-tree", timeoutMs: 8_000,
      command: {kind: "direct", file: process.execPath, args: ["-e", parent, marker]},
    });
    expect(accepted.ok).toBe(true);
    const taskId = (accepted.data as {taskId: string}).taskId;
    await until(() => daemonRequest(root, {action: "output", taskId, stream: "stdout"}),
      result => (result.data as {text?: string} | undefined)?.text?.includes("SPAWNED") === true);
    const requested = await daemonRequest(root, {action: "cancel", taskId});
    expect(requested).toMatchObject({ok: true, data: {taskId, state: "canceling"}});
    const completed = await until(() => daemonRequest(root, {action: "get", taskId}),
      result => (result.data as {state?: string} | undefined)?.state === "canceled");
    expect(completed.data).toMatchObject({state: "canceled", outcome: "known"});
    // A detached worker writes a completion manifest; the daemon can import it after closure.
    expect((await daemonRequest(root, {action: "result", taskId})).data)
      .toMatchObject({state: "canceled", manifest: {canceled: true, signal: "SIGTERM"}});
    await daemon.close();
  ...
  it("persists a cancel request across coordinator shutdown and imports the worker's cancel evidence", async () => {
    const {root, daemon} = await fixture();
    const accepted = await daemonRequest(root, {
      action: "submit", workspace: root, idempotencyKey: "recover-cancel", timeoutMs: 8_000,
      command: {kind: "direct", file: process.execPath,
        args: ["-e", "console.log('READY_TO_CANCEL');setInterval(()=>{},100)"]},
    });
    const taskId = (accepted.data as {taskId: string}).taskId;
    await until(() => daemonRequest(root, {action: "output", taskId, stream: "stdout"}),
      result => (result.data as {text?: string} | undefined)?.text?.includes("READY_TO_CANCEL") === true);
    expect((await daemonRequest(root, {action: "cancel", taskId})).data)
